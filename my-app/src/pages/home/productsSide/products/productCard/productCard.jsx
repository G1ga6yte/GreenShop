...
function ProductCard({ productInfo, addedInCart, setMessage, setMessageStatus}) {
    const {cartProducts, setCartProducts, accountInfo} = useCartContext()
    const navigate = useNavigate();

    const handleAddToCart = async (e) => {
        e.stopPropagation()
        if (cartProducts.includes(productInfo.productID)){
            setCartProducts(prevItems => prevItems.filter(item => item !== productInfo.productID))
        } else {
            setCartProducts(prevItems => [...prevItems, productInfo.productID])
        }

        axiosInstance.post("/cart/addRemoveCart", {
    ...
    const handleSaveInFavorites = async (e) => {
        e.stopPropagation()
    }

    return (
        <div onClick={()=>navigate(`/product/${productInfo.productID}`)} className="ProductCart" >
            <div className="imgBlock">
                <img src={productInfo.images[0]} className="prdImg" alt="" />
                <div className="buttonsBlock">

                    <button onClick={handleAddToCart}
                            className={`btn btn1 ${addedInCart && "activeBtn"}`}>
                    ...
                    <button onClick={handleSaveInFavorites} className="btn btn2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    ...
                    <Link onClick={(e)=>e.stopPropagation()} to="/" className="btn btn3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
            ...
            <div className="textInfo">
                <p className="name">{productInfo.productName}</p>
                <div className="priceBlock">
                    <span className="price">${productInfo.productPrice}.00</span>
                    <span className="oldPrice">{productInfo.oldPrice ? productInfo.oldPrice !== 0 ? `$${productInfo.oldPrice}.00` : "" : ""}</span>
                </div>
            </div>
        </div>
    )
